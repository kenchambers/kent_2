...
export function Terminal() {
  const [messages, setMessages] = useState([
    {
      type: "system",
      content: "Kent AI Agent initialized. Type your message below.",
    },
  ]);
  ...
  useEffect(() => {
    // Get or create a persistent user_id from localStorage
    let storedUserId = localStorage.getItem("kent_user_id");
    if (!storedUserId) {
      storedUserId = `user_${Date.now()}_${Math.random()
        .toString(36)
        .substr(2, 9)}`;
      localStorage.setItem("kent_user_id", storedUserId);
    }
          ...
          if (line.startsWith("data: ")) {
            try {
              ...
              if (data.type === "thinking") {
                ...
                setThinkingSteps((prev) =>
                  prev.map((step) => ({ ...step, isNew: false, fading: true }))
                );
      ...
      <div className="border-t border-white/10 p-4">
        <form onSubmit={sendMessage} className="flex items-center space-x-2">
          ...
          <input
            ...
            placeholder={
              isStreaming ? "Kent is processing..." : "Type your message..."
            }
